...
public static class NumberUtilities
{
    ...
    /// <returns>
    ///     Magnitude of the number. Negative when the number is less than 1 and positive when the number is greater,
    ///     than 1.
    /// </returns>
    ...
    ///     Minimum number of significant figures to display if value is
    ///     less than 1.
    /// </param>
    ...
    public static string ToNumberString(double value, int significantFigures = 4, bool removeTrailingZeros = true)
    {
        ...
        if (!removeTrailingZeros) return formattedValue;

        // Remove trailing zeros
        if (formattedValue.Contains('.')) formattedValue = formattedValue.TrimEnd('0').TrimEnd('.');
    ...
    public static string ToAutoString(double value, int significantFigures, bool latex = false)
    {
        ...
        if (exponent == 0) return ToNumberString(value, significantFigures);

        if (latex) return $"{ToNumberString(scaledValue, significantFigures)} \\times 10^{{{exponent}}}";
