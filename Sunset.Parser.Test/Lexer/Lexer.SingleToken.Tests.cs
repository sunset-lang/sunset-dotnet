...
public class LexerSingleTokenTests
{
    ...
    public void GetNextToken_SingleCharacterTokens_HasCorrectValues()
    {
        ...
        {
            var lex = new Language.Lexer(key.ToString(), false);
    ...
    public void GetNextToken_DoubleCharacterTokens_HasCorrectValues()
    {
        ...
        foreach (var key in expected.Keys)
        {
            var lex = new Language.Lexer(key.firstCharacter + key.secondCharacter.ToString(), false);
            var token = lex.GetNextToken();
            Assert.That(token.Type, Is.EqualTo(expected[key]));
        }
    }
    ...
    {
        var lex = new Language.Lexer("123.456", false);
        var token = lex.GetNextToken();
    ...
    {
        var lex = new Language.Lexer("123.45678e3", false);
        var token = lex.GetNextToken();
    ...
    {
        var lex = new Language.Lexer("-123", false);
        var token = lex.GetNextToken();
    ...
    {
        var lex = new Language.Lexer("123456", false);
        var token = lex.GetNextToken();
        Assert.That(token.Type, Is.EqualTo(TokenType.Number));
    ...
    {
        var lex = new Language.Lexer("123.456.789", false);
        var token = lex.GetNextToken();
    ...
    {
        var lex = new Language.Lexer("\"Hello, world\"", false);
        var token = lex.GetNextToken();
    ...
    {
        var lex = new Language.Lexer("\"\"\"Hello, world\r\nHow are you doing today?\"\"\"", false);
        var token = lex.GetNextToken();
    ...
    {
        var lex = new Language.Lexer("test", false);
        var token = lex.GetNextToken();
