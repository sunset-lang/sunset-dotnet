...
{
    private readonly MarkdownQuantityPrinter _quantityPrinter;
    ...
    {
        Settings = new();
        _quantityPrinter = new MarkdownQuantityPrinter(Settings);
    ...
    public string PrintReport(ReportSection section)
    {
        ...
        if (Settings.PrintTableOfContents)
        {
            builder.Append(PrintTableOfContents(section));
        }
    ...
    )
    {
        ...
        foreach (ReportSection subsection in section.Subsections)
        {
    ...
    /// <param name="parentHeading">Parent heading that owns this subsection. Defaults to empty string to indicate
    /// root parent.</param>
    ...
    /// <summary>
    /// Prints the information of the provided Quantity objects into a dot point list and appends it to the provided
    /// StringBuilder <paramref name="builder"/>
    /// </summary>
    /// <param name="quantities">Quantities to be printed.</param>
    /// <param name="builder">StringBuilder used to append the list to.</param>
    private void PrintQuantityInformation(List<IQuantity> quantities, StringBuilder builder)
    {
        builder.AppendLine("Where: ");
        var quantitiesToPrint = new HashSet<IQuantity>();
        foreach (var quantity in quantities)
        {
            var quantitiesInExpression = quantity.GetDependentQuantities();
            foreach (IQuantity dependentQuantity in quantitiesInExpression)
            {
                quantitiesToPrint.Add(dependentQuantity);
            }
        }

        foreach (var quantity in quantitiesToPrint)
        {
            var quantityInformation = _quantityPrinter.ReportQuantityInformation(quantity);
            if (quantityInformation != "")
            {
                builder.AppendLine(_quantityPrinter.ReportQuantityInformation(quantity));
            }
        }
    }
